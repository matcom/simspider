...
import sys
from Redist.antlr3 import *
from Redist.antlr3.compat import set, frozenset
         
#docstring
"""Parser for reading FCL by the pyfuzzy package."""
__revision__ = "$Id: FCLParser.py,v 1.5 2009/09/27 18:20:00 rliebscher Exp $"
import Redist.pyfuzzy.System
import Redist.pyfuzzy.InputVariable
import Redist.pyfuzzy.OutputVariable
import Redist.pyfuzzy.Adjective
import Redist.pyfuzzy.set.Polygon
import Redist.pyfuzzy.set.Singleton
import Redist.pyfuzzy.defuzzify
import Redist.pyfuzzy.defuzzify.Dict
import Redist.pyfuzzy.fuzzify
import Redist.pyfuzzy.fuzzify.Plain
import Redist.pyfuzzy.fuzzify.Dict
import Redist.pyfuzzy.operator.Not
import Redist.pyfuzzy.operator.Input
import Redist.pyfuzzy.operator.Compound
import Redist.pyfuzzy.Rule
import Redist.pyfuzzy.norm.Min
import Redist.pyfuzzy.norm.Max
...
_operators = {
    "AND":Redist.pyfuzzy.norm.Min.Min(),
    "OR":Redist.pyfuzzy.norm.Max.Max()
    }
    ...
    def function_block_declaration(self, ):
        ...
        try:
            try:
                ...
                self._state.following.pop()
                #action start
                self.System = Redist.pyfuzzy.System.System(description=((function_block_name1 is not None) and [self.input.toString(function_block_name1.start,function_block_name1.stop)] or [None])[0]);
                #action end
    ...
    def var_decl(self, output_var):
        ...
        try:
            try:
                ...
                if output_var == 0:
                    var=Redist.pyfuzzy.InputVariable.InputVariable()
                    if type4 is not None:
                        # set fuzzification method to dictionary input
                        var.fuzzify = Redist.pyfuzzy.fuzzify.Dict.Dict();
                        # create adjectives for all struct members
                        for i in type4:
                            var.adjectives[i] = Redist.pyfuzzy.Adjective.Adjective(Redist.pyfuzzy.set.Set.Set())
                    else:
                        # default is the plain fuzzification
                        var.fuzzify = Redist.pyfuzzy.fuzzify.Plain.Plain();
                else:
                    var = Redist.pyfuzzy.OutputVariable.OutputVariable()
                    if type4 is not None:
                        # set defuzzification method to dictionary output
                        var.defuzzify = Redist.pyfuzzy.defuzzify.Dict.Dict();
                        # create adjectives for all struct members
                        for i in type4:
                            var.adjectives[i] = Redist.pyfuzzy.Adjective.Adjective(Redist.pyfuzzy.set.Set.Set())
    ...
    def linguistic_term(self, var_name):
        ...
        try:
            try:
                ...
                self.System.variables[var_name].adjectives[((term_name10 is not None) and [self.input.toString(term_name10.start,term_name10.stop)] or [None])[0]] = Redist.pyfuzzy.Adjective.Adjective(membership_function11);
        ...
        try:
            try:
                ...
                if alt20 == 1:
                    ...
                    self._state.following.pop()
                    #action start
                    set =  Redist.pyfuzzy.set.Singleton.Singleton(float(((numeric_literal14 is not None) and [self.input.toString(numeric_literal14.start,numeric_literal14.stop)] or [None])[0]))
                    #action end
    ...
    def points(self, ):
        ...
        try:
            try:
                ...
                #action start
                set =  Redist.pyfuzzy.set.Polygon.Polygon(p)
                #action end
            ...
            try:
                ...
                while True: #loop28
                    ...
                    if alt28 == 1:
                        ...
                        input =  Redist.pyfuzzy.operator.Compound.Compound(getOperator(op.text),input,s2)
        ...
        try:
            try:
                ...
                if alt29 == 1:
                    ...
                    #action start
                    input =  Redist.pyfuzzy.operator.Not.Not(condition17)
                    #action end
        ...
        try:
            try:
                ...
                if alt32 == 1:
                    ...
                    input = Redist.pyfuzzy.operator.Input.Input(self.System.variables[((variable_name19 is not None) and [self.input.toString(variable_name19.start,variable_name19.stop)] or [None])[0]].adjectives[((term_name20 is not None) and [self.input.toString(term_name20.start,term_name20.stop)] or [None])[0]])
                    if x is not None:
                        input = Redist.pyfuzzy.operator.Not.Not(input)
                    ...
                    input =  Redist.pyfuzzy.operator.Compound.Compound(norm,c4,c5)
    ...
    def rule(self, block_name):
        ...
        try:
            try:
                ...
                self.System.rules[block_name+'.'+Integer_literal24.text] = Redist.pyfuzzy.Rule.Rule(adjective,input,certainty=certainty)
